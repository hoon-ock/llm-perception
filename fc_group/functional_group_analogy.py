...
def plot_diff_vector_quiver_3d(mean_diff_by_group, layer, output_path, highlight_pair=None):
    """3D quiver plot of every group's mean diff-vector as an arrow from the
    ...
    ax.set_title(f'Mean diff-vectors in 3D (layer {layer})\n'
...
def run_layer(layer, df, alkane_index, entity_type, activations_dir, num_null_samples, output_dir, rng):
    ...
    if analogy_rows:
        save_long_format_csv(
            ...
            os.path.join(output_dir, 'data', f'second_order_analogy_layer_{layer}.csv'),
        )
